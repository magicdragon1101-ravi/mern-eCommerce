...
const PlaceOrderScreen = () => {
  ...
  useEffect(() => {
    if (success) {
      router.push({ pathname: "/order/[id]", query: { id: order._id } });
    }
  }, [success]);
            ...
            {cartItems.lenght === 0 ? (
              ...
              <div>
                {cartItems.map((item, index) => (
                  <div
                    key={index}
                    className="grid gap-2 grid-cols-6 items-center ml-4 mb-4 pb-4 border-gray-300 border-b"
                  >
                    ...
                    <div className="col-start-2 col-span-3 text-gray-800">
                      <Link
                        href={{ pathname: "/product/[id]", query: { id: item.product } }}
        ...
        <div className="col-start-9 col-span-4">
          <div className="border-gray-300 border pb-4 mt-4">
            <div className="border-gray-300 border-b">
              <h3 className="px-6 py-4 text-3xl uppercase tracking-widest text-gray-700 font-semibold">
                Order Summary
              </h3>
            </div>
            <div className="flex justify-between pb-4 border-gray-300 border-b">
              ...
              <div>
                <p className="pt-4 px-6">${itemsPrice}</p>
              </div>
            </div>
            <div className="flex justify-between pb-4 border-gray-300 border-b">
              <div>
              ...
              <div>
                <p className="pt-4 px-6">${shippingPrice}</p>
              </div>
            </div>
            <div className="flex justify-between pb-4 border-gray-300 border-b">
              <div>
              ...
              <div>
                <p className="pt-4 px-6">${taxPrice}</p>
              </div>
            </div>
            <div className="flex justify-between">
              ...
              <div>
                <p className="pt-4 px-6">${totalPrice}</p>
              </div>
            </div>
            <div>
              {loading && <Loader />}
              {error && (
                <Message color="red" variant="600">
                  {error}
                </Message>
              )}
              {success && (
                <Message color="green" variant="700">
                  Order Success
                </Message>
              )}
            </div>
          ...
          <div className="py-4 flex justify-center border-gray-300 border-b border-l border-r">
            <button className="px-20 py-2 text-white bg-black uppercase" onClick={placeOrderHandler}>
              Place Order
            </button>
          </div>
        </div>
