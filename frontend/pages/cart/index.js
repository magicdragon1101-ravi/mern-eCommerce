      ...
      <div className="custom-container md:grid md:gap-8 md:grid-cols-6">
            ...
            cartItems.map((item) => (
              <div key={item.product}>
                <div className="grid gap-6 grid-cols-5 justify-items-center items-center pb-2 mb-4 border-gray-200 border-b text-gray-800">
                  ...
                  <div>
                    <button
                      className="p-4 hover:bg-gray-400 transition duration-500 ease-in-out"
        ...
        <div className="col-start-5 col-span-2">
          <div className="border-gray-300 border px-6 pb-4 mt-4">
            <h3 className="pt-4 text-3xl uppercase tracking-widest text-gray-700 font-semibold">
              Subtotal ({cartItems.reduce((acc, item) => acc + item.qty, 0)}) items
            </h3>
            <p className="pt-4">${cartItems.reduce((acc, item) => acc + item.qty * item.price, 0).toFixed(2)}</p>
          </div>
          <div className="py-4 flex justify-center border-gray-300 border-b border-l border-r">
            <button className="px-20 py-2 text-white bg-black uppercase" onClick={checkoutHandler}>
              Proceed to Checkout
            </button>
          </div>
        </div>
